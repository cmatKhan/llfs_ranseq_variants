...
#' rna and dna
set_overlap_filter = function(gds_list, chr){
  # extract granges
...
tidy_geno_mat = function(gds_obj,source,sample,chr){
  ...
  total_depth = tryCatch(
    seqGetData(gds_obj,'annotation/format/DP')[1,],
    error = function(e){NA})
  ad = tryCatch(
    seqGetData(gds_obj,'annotation/format/AD'),
    error = function(e){NA})
  alt_depth_colname = paste(source,'alt_depth',sep="_")
  alt_depth = tryCatch(
    ad$data[1,seq(2,length(ad$length)*2,2)],
    error = function(e){NA})
  ...
  genotype_qual = tryCatch(
    seqGetData(gds_obj,'annotation/format/GQ')[1,],
    error = function(e){NA})
...
parser <- add_option(parser, c("--dna"), type="character",
                     default="",
                     help="path to dna gds lookup with cols gds, chr. csv format",
                     metavar="'/path/to/gds_lookup.csv'")
...
opt = if(TEST){
  parse_args(parser,
           args = c(
             ...
             "--rna_sample=20790",
             "--dna_sample=20790",
             "--dna=data/genoarr_lookup.tsv",
             "--rna=/mnt/scratch/llfs_rna_dna_compare_test/tmp_vcf_gds/20790_visit2_T1.haplotypecaller.filtered.gds"))
} else{
  parse_args(parser)
}
...
dna_genoarr_df = read_tsv(opt$dna) %>%
  filter(chr %in% paste0('chr',seq(1,22)))

all_chr_df = foreach(
  dna_lookup = iterators::iter(dna_genoarr_df, by='row'),
  .combine = rbind,
  .verbose = TRUE
) %do% {
  if(opt$verbose){
    message(sprintf("rna_sample: %s; dna_sample: %s; chr: %s",
                  opt$rna_sample,opt$dna_sample,str_remove(dna_lookup$chr,'chr')))
  }

  # open gds files
  gds_list = list(
    rna = seqOpen(opt$rna,allow.duplicate = TRUE),
    dna = seqOpen(dna_lookup$gds,allow.duplicate = TRUE)
  )

  init_rna_var_n = set_initial_gds_filters(
    gds_list$rna,
    str_remove(dna_lookup$chr,'chr'),
    opt$rna_sample)
  ...
  overlap_var_n = set_overlap_filter(gds_list, str_remove(dna_lookup$chr,'chr'))

  if(overlap_var_n <= 0){

    # close gds obj
    x = map(gds_list, seqClose)

  } else{

    # extract data
    compare_df = tidy_geno_mat(gds_list$rna,'rna',opt$rna_sample,str_remove(dna_lookup$chr,'chr')) %>%
      left_join(tidy_geno_mat(gds_list$dna,'dna',opt$dna_sample,str_remove(dna_lookup$chr,'chr')),by=c('chr','pos')) %>%
    ...
    comparison_summary_fltr = compare_df %>%
      filter(rna_total_depth >= depth_fltr,
             rna_gq >= gq_fltr) %>%
    ...
    if(dna_lookup$chr == 'chr16'){
      message('here!')
    }

    tibble(
      chr = str_remove(dna_lookup$chr,'chr'),
      rna_sample = opt$rna_sample,
...
file_name = paste('visit', opt$rna_visit, opt$dna_sample, sep = "_")

# write full comparison df if -f is set
if(opt$write_full_comparison){
  write_csv(all_chr_df,
            file.path(opt$output_prefix,
                      output_dir,
                      paste0(file_name,"_all_chr.csv")))
}

# write summarized match metric
all_chr_df %>%
  group_by(rna_sample,rna_visit,dna_sample) %>%
  summarize_at(c('rna_var_n','dna_var_n','overlap_var_n', 'overlap_unfltr',
                 'overlap_fltr','n_match_unfltr','n_match_fltr',
                 'homo_expr_cand_fltr'), sum) %>%
  write_csv(file.path(opt$output_prefix,
                      output_dir,
